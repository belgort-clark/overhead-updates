...
- Fixed: the logbook could name a finance company as an operator. A Diamond DA40 was filed under "Wingsleasing", and the lowest aircraft ever to come over under "Shelby Patrick B Trustee". Both hold title rather than fly anything. The check that already refused those names only ever protected airliners, because it fell back to the registered owner whenever the callsign was not an airline's, which is every light aircraft. The logbook also stores the name it was given at the time, so rows filed before the check existed kept theirs; those are filtered now as well, and a record giving only the registration and the type is telling the truth
- Finding an ATC feed takes fewer steps. The instructions taught the long way round, downloading a playlist file and opening it from Finder, and offered the short way as an afterthought. Right-click a feed's **launches your MP3 player** icon, choose **Copy Link**, and Overhead spots it on your clipboard the moment you come back: that is steps 1 and 2 now. The file route is still there, still the one that arrives already named, and in Safari **Open "safe" files after downloading** makes it a single click
